"prepare_glove"<-function(item_list_text,glove_skipgram_clause=T,ws=21,split2,full_tcm_clause=F,dimensions=200){
  ...
  if(glove_skipgram_clause==T){
    contents=item_list_text$text[split2==T]
    ...
    vocab_pruned=vocab[match(colnames(item_list_text$dtm),vocab[,1]),]
    ...
    if(full_tcm_clause==T){
      tcm_glove=crossprod(as.matrix(item_list_text$dtm[split2==T,]))
    }else{
      tcm_glove=item_list_text$tcm
